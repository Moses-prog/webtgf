...
async def text_handler(event):
    ...
    if not state:
        ...
        if "t.me/" in event.text:
            ...
            import asyncio
            msg = await event.reply("⏳ Fetching post and media using your connected account...")
            ...
            for _ in range(15):
                ...
                if res:
                    ...
                    media_path = ud.get("manual_extract_media")
                    ud["manual_extract_media"] = ""
            ...
            await msg.edit("❌ Timeout fetching link. Is your background forwarding engine running?")
            return
    ...
    if state == "waiting_for_sources" or state == "waiting_for_targets":
        ...
        elif getattr(event.message, 'video', None) or getattr(event.message, 'document', None):
            m = await event.respond(" \u23f3 Downloading video, please wait...")
            path = await event.message.download_media(file=f"database/images/{chat_id}_vid_override")
